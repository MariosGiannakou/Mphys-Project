...
const int sims=1;
const int Tmax=120000;
const double equil_tim=100000; //time for an equilibration must be even, Tmax>equil_tim
...
double dx=pow(dt*2*diff,2);


const int maxtemp=10.0;//Cycling temp from 1 to 5 and back
...
int main(void){
  ...
  double dx2[Tmax];
  double cor[Tmax];
  int tau_interval=200;
  int aver_inter=40000;
  int tau[tau_interval];
  ...
  for(int j=0;j<sims;j++){
    ...
    // for(int i=0;i<tau_interval;i++){
    //   tau[i]=i-tau_interval/2;
    //   for(int l=(Tmax/2-aver_inter);l<(Tmax/2+aver_inter);l++) {
    //     cor[i]+=dx1[l+tau[i]]*dx2[l];
    //   }
    //   cor[i]=cor[i]/(2*aver_inter);
    // }
  ...
  //
  // for(int k=0;k<Tmax;k++) {
  //   writeposit <<time_arr[k]<<" "<< pos1[k] <<" "<<pos2[k] <<" "<<dis12[k] <<" "<< endl;
  // }
  //
  // writeposit.close();
  ...
  //
  // for(int k=0;k<tau_interval;k++) {
  //   writecor <<tau[k]<<" "<< cor[k]<< endl;
  // }
  // writecor.close();

  stringstream displa;
  displa << "displacements.dat";
  ofstream writedis;
  writedis.open(displa.str().c_str());

  for(int k=0;k<Tmax;k++) writedis << time_arr[k] << " " << dx1[k]<< " " << dx2[k] <<endl;
  writedis.close();
